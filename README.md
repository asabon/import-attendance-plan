...
- **処理順序**: 複数のメール（スレッド）を一度に処理する場合、受信日時の**古いメールから順に**カレンダーへ反映されます。これにより、後から送信された指示で正しくスケジュールが上書きされます。
- **再処理手順**: 二重取り込みを防ぐため、一度処理したメールの `Message-ID` はログ用スプレッドシート（G列）に自動で記録され、次回以降の実行ではスキップされます。もし一度処理したメールを再度処理させたい場合は、以下の両方を行ってからスクリプトを実行してください。
  1. 対象メールを受信トレイに戻す（アーカイブを解除する）
  2. ログスプレッドシートから該当するメールの `Message-ID`（またはその行）を削除する
